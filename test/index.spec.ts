...
describe('makeUniversalApp', () => {
  ...
  it('throws an error if asar is only detected in one arch', async () => {
    ...
    await expect(
      makeUniversalApp({
        x64AppPath: path.resolve(appsDir, 'X64Asar.app'),
        arm64AppPath: path.resolve(appsDir, 'Arm64NoAsar.app'),
        outAppPath: out,
      }),
    ).rejects.toThrow(
  ...
  describe('force', () => {
    it('throws an error if `out` bundle already exists and `force` is `false`', async () => {
      ...
      await expect(
        makeUniversalApp({
          x64AppPath: path.resolve(appsDir, 'X64Asar.app'),
          arm64AppPath: path.resolve(appsDir, 'Arm64Asar.app'),
          outAppPath: out,
        }),
  ...
  describe('asar mode', () => {
    ...
    it(
      ...
      async () => {
        ...
        await makeUniversalApp({
          x64AppPath: path.resolve(appsDir, 'X64Asar.app'),
          arm64AppPath: path.resolve(appsDir, 'Arm64AsarExtraFile.app'),
          outAppPath: out,
        });
        await verifyApp(out);
      },
  ...
  describe('no asar mode', () => {
    it(
      ...
      async () => {
        ...
        await makeUniversalApp({
          x64AppPath: path.resolve(appsDir, 'X64NoAsar.app'),
          arm64AppPath: path.resolve(appsDir, 'Arm64NoAsar.app'),
          outAppPath: out,
        });
        await verifyApp(out);
